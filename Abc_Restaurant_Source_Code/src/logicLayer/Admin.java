...
public class Admin {
    ...
    public void editHotel(String Name, String Loc, String ChangeType, String Change , Vector<Restaurant> Hots, writerAndReader ReadAndWrite){
        for(int i=0;i<Hots.size();++i){
            if(Name.equals(Hots.get(i).getName()) && Loc.equals(Hots.get(i).getLocation())){
                if(ChangeType=="TotalRoooms"){
                    ...
                    Hots.get(i).setSingletable(Change);
                }
                else if(ChangeType=="DoubleRooms"){
                    Hots.get(i).setDoubletable(Change);
                }
                else if(ChangeType=="SingleRoomPrice"){
                    Hots.get(i).setSingletableprice(Change);
                }
    ...
    public void DeleteHotel(String Name, String Loc, Vector<Restaurant> Hots, writerAndReader ReadAndWrite){
        int t=-1;
        for(int i=0;i<Hots.size();++i){
            //Replacing IDs and Also Finding Index to Delete
            if(t!=-1){
